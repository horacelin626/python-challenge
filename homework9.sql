...
#4b. List last names of actors and the number of actors who have that last name, but only for names th s.at are shared by at least two actors
select last_name, count(last_name) as 'count'
from actor
group by last_name
having count > 2;
...
select title, count(film.film_id) as 'Count_of_Rented_Movies' from film
